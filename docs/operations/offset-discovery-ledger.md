...
The session machinery is stable and repeatable: G2 is twice proven live, the
write-observation is consistent run-over-run (same dominant write sites, same
18-in-window count), and the poll's read-failure pattern is the only open
variable — improving 19/24 -> 22/24 across sessions. The remaining step is
unchanged: a further approved session targeting a 24/24 positive poll (the
avatar-helper pointer-race is battle-segment dependent; a different segment
or entity may avoid it). Do not change the resolver, broaden reads, or
promote the offset table; `HardwareAtomicReadProven` and
`stableRootLiveRepeatabilityProven` remain false.
