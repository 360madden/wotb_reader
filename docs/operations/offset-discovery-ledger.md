...
| Next planned session | `OD-RECOVERY-086` — batch N-entity rehearsal (resolver-path consolidation item 6/3; G0 publication OD-RECOVERY-083/084 is applied and retained in history): `scripts/invoke-batch-rehearsal.ps1 -LiveAcquire -Times 90,150,220 -FailOnMiss` on Oasis Palms — full-roster batch dumps through `/discover/entity-regions` per replay time (one G2 clock attestation per batch, fail-closed), cross-checked against decoded positions (tool proven 42/42 on real data), measuring the read-pass window (item 4 → feeds item 7). Pre-staged: design `docs/operations/batch-entity-read-design.md`, coordinator `ReadEntityRegionsAsync` + endpoint + 11 tests, driver + `scripts/python/batch-rehearsal-crosscheck.py`. Closes the X2 rehearsal; the L1 HP / L2 facing / CAM-001 v7 live gates follow in pre-staged order. |
