...
| Next planned session | `OD-RECOVERY-044` (static milestones `OD-039..043-STATIC` complete: the two 'root candidates' are members of a repeating 0x50-byte record family, NOT gameplay roots; `0x037F3054` is the shared RTTI `type_info` vftable (every TypeDescriptor's pVFTable points at it — confirmed td@0x03DB5120, explaining the 48,609 `.data` refs); **`--vtables` names 17,133 of 18,721 vtables** (MSVC x86 stores the mangled name inline at td+8); `--vtable-root`/`--table-map` modes added and `.data 0x03B7E198` reclassified as the **DAVA `AnyFn` invoker vtable table** (24 named `StaticAnyFnInvoker<lambda>` at modal stride 0x2C binding TankComponent/AimingPointComponent/Scene/Entity — component event subscriptions; each vtable has exactly 1 `.data` root = its array entry, internally-closed set; runtime write at 0x03104FAB repoints entry[0]) — **dispatch infrastructure, NOT a gameplay root**, do NOT probe it as a singleton). Highest-value live run: the proven invocation `-SnapshotMaxBytes 402653184 -MaxRounds 40 -HoldAfterRollSeconds 240` with the operator present during the held green window — the 11-survivor replayTime set is usable for interactive Find-what-writes even without ≤10 — optionally with a **delta-compare pilot** feeding a replay-derived position delta to break the plateau; alternatively a content-distinct second replay for BLK-0019, or investigating the replay-start flake root cause (the game dies quietly ~2s after `LoadGameScene` ends with no crash dump) |
