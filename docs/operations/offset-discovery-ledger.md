...
| Next planned session | DONE (2026-08-10, OD-RECOVERY-083 + OD-RECOVERY-084): the operator-approved G0 publication is applied — `playerPositionX/Y/Z` are `Verified` via the module-rooted position-ring chain (`chains` section, offsets stay 0), evidence appended, approvals set, `numericOffsetPublication: true` — and the 2nd-generation WALKABLE form is applied (OD-RECOVERY-084): the published chains now carry `inlineOffset` + `entityLookup` + INLINE `ringIndex` semantics so the chain walker reads the published table directly (proven by `Walk_PublishedTableChains_*` resolver-equivalence tests; fidelity check enforces draft ↔ published identity). All gates green (`offset_check.py` chains-validated 3 fields + fidelity, evidence report clean, links 112/112, exclusion test passed, `validate.ps1` exit 0). What remains: nothing live; the resolver + read surface are untouched and the legacy observation path still emits position nulls (chained fields excluded — pinned by `ChainedFields_AreExcludedFromObservationReads`). Post-publication regression plan: `docs/operations/g0-post-publication-regression.md`. Note for any future live run: pass `-PriorResultPaths` space-separated or comma-joined (the wrapper now normalizes both; comma-joined was previously bound as a single path). |
