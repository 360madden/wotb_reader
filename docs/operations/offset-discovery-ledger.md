...
| Next planned session | **CAM-001 v7 RE-ATTEMPT — screen-vs-pose check** (root cause ISOLATED 2026-08-11, session `019ff25b` + 3 probes: the GameCamera instance is UNIQUE and the chain reaches it (wrong-instance FALSIFIED); class identity across CAM-004/today is the same RVA under different ASLR (wrong-class FALSIFIED); the object-window scan found NO near-tank pose anywhere in +0x00..0x200 (wrong-field FALSIFIED) — posA holds a real live pose that tracks the tank x exactly but sits ~50 m above/behind with level pitch, i.e. a NON-chase camera STATE this launch. The remaining discriminator is MODE vs wrong-pose: capture the rendered game window and compare against posA — high-view on screen ⇒ posA right, W2S consumes it as-is; chase-view on screen ⇒ field re-derivation. Rendering-only decision, no resolver/offset-table change.) **OD-RECOVERY-090 is DONE (2026-08-11): HONEST-NEGATIVE** — no damage-dealt counter in the 320-byte entity-base region (session `019ff250`, 50 dumps, all 6 event windows + controls; top candidate `+0x3C` score 0.833 demoted by flatness 0.091 — it is the already-measured position-copy float; damage totals corrected to 6/752). Re-attempt needs a wider region / sibling-record sweep. OD-RECOVERY-089 is DONE (2026-08-11): **L2 facing Phase-4 HIT** — ring-record `+0x30` agrees on Dead Rail (56/56, score 1.0, flatness 1.0), `twoReplayRepeatability = true`; the at-session honest-negative was a matcher limitation (one-directional shared lag vs the opposite-sign per-dump G2 label skew), fixed additively by the per-dump bidirectional lag path and proven on both replays. The yaw publication package is READY (`docs/operations/g1-yaw-publication-draft.md` — operator approval + gate run only); the Phase-4 two-replay HP rule (Dead Rail victim 2549399) still gates HP publication; item 7 (hardware atomicity) stays LAST. |
