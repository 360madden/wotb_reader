...
| Next planned session | `OD-RECOVERY-042` (static milestone `OD-041-STATIC` complete: **`0x03FA0C74` is NOT a standalone gameplay root — it is the +0x04 member of record[1] in a repeating 0x50-byte `.data` record family** `{0x00404064 (.text member-fn), <runtime slot>, 0x037F3054 (.rdata EH handler table), 0, …}` based at `0x03FA0C20`; `0x037F3054` is an **MSVC EH handler/funclet table** (0xFFFFFFFF sentinel + interleaved {state, code-ptr} pairs into `.text` 0x02CF0D70+, 113 FuncInfo magics within ±0x2000) referenced from **48,609 4-aligned `.data` slots (0 unaligned)** — a massively-shared infrastructure pointer, not a singleton; `--record-map 0x03FA0C20,0x50,10` maps 10 records / 9 runtime slots; the two OD-039/040 'root candidates' are reclassified as **EH/handler record members**, not gameplay state — live-probing them as singleton roots is ruled out without a changed hypothesis). Highest-value live run: the proven invocation `-SnapshotMaxBytes 402653184 -MaxRounds 40 -HoldAfterRollSeconds 240` with the operator present during the held green window — the 11-survivor set is usable for interactive Find-what-writes even without ≤10 — optionally with a **delta-compare pilot** feeding a replay-derived position delta to break the plateau; alternatively a content-distinct second replay for BLK-0019, or investigating the replay-start flake root cause (the game dies quietly ~2s after `LoadGameScene` ends with no crash dump) |
