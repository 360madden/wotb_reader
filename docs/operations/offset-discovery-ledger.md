...
| Next planned session | **CAM-001 v7 RE-ATTEMPT — isolate first** (2026-08-11: two launches returned `camera-state-found-unverified-offset` — camera identity gates PASS but posA read ~120 m from the tank with level pitch while pitch-to-tank ≈ −46°; tank source verified CORRECT both times (2.4 m match to decoded); the memory camera is NOT aimed at the tank on the flipped session-controller phase; root cause unresolved, candidates documented in `docs/operations/cam001-v7-evidence-template.md`, no guesses promoted). Next session must fingerprint the GameCamera instance (vftable hex + sibling pointers) and compare against CAM-004's session-controller variant before re-attempting the look-at check. **OD-RECOVERY-090 is DONE (2026-08-11): HONEST-NEGATIVE** — no damage-dealt counter in the 320-byte entity-base region (session `019ff250`, 50 dumps, all 6 event windows + controls; top candidate `+0x3C` score 0.833 demoted by flatness 0.091 — it is the already-measured position-copy float; damage totals corrected to 6/752). Re-attempt needs a wider region / sibling-record sweep. OD-RECOVERY-089 is DONE (2026-08-11): **L2 facing Phase-4 HIT** — ring-record `+0x30` agrees on Dead Rail (56/56, score 1.0, flatness 1.0), `twoReplayRepeatability = true`; the at-session honest-negative was a matcher limitation (one-directional shared lag vs the opposite-sign per-dump G2 label skew), fixed additively by the per-dump bidirectional lag path and proven on both replays. The yaw publication package is READY (`docs/operations/g1-yaw-publication-draft.md` — operator approval + gate run only); the Phase-4 two-replay HP rule (Dead Rail victim 2549399) still gates HP publication; item 7 (hardware atomicity) stays LAST. |
