...
| Next planned session | G1, G2, and G3 are CLOSED (OD-RECOVERY-082: stored v4 aggregate 24/24 `stable-resolver-positive` with `allConsistentDoubleRead=true`; G2 `sameDecodedClockProven=true` 4th confirmation; G3 on the positive verdict + directly-validated OD-075/076 priors). The G0 publication review is executed with verdict PROMOTE-READY (conditional). What remains: (1) the OPERATOR-APPROVED table change — `memory-offsets/11.19.0.10.json`: playerPositionX/Y/Z `status: Candidate -> Verified` with the chain-form values (schema-representation decision item: the position is a pointer chain, not a single RVA — record the resolver-layout chain in the evidence + the root RVA as the offset value, or a schema extension) + evidence + `leadApproved`/`decoderAuditorApproved`; (2) post-edit gates: `tools/report-offset-evidence.ps1 -GameVersion 11.19.0.10`, `python scripts/python/offset_check.py --check-schema`, regenerate `offline/file-tree.md`, `scripts/validate.ps1`; (3) commit the table + ledger + handoff as one change with `numericOffsetPublication: true`. The exact operator-ready change (schema decision: additive `chains` section, offsets stay 0, fieldValidation → Verified, validator + pack-doc updates, post-edit gates) is drafted in `docs/operations/g0-offset-table-draft.md`. No further live sessions are required unless a table edit is rejected. Note for any future live run: pass `-PriorResultPaths` space-separated or comma-joined (the wrapper now normalizes both; comma-joined was previously bound as a single path). |
