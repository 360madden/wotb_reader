...
| Next planned session | Cross-replay continuous polling is proven (OD-075 Dead Rail + OD-076 Oasis Palms, both 24/24). Remaining gates are tracked in `docs/operations/offset-promotion-checklist.md`: wire `stableRootLiveRepeatabilityProven` (runner-only), populate `replay_clock_segments` (decoder offline work), then hardware-atomic read proof and same-decoded-clock correlation (new approved live session). Do not change the resolver, broaden reads, or promote the offset table before those gates close. |
