...
| Next planned session | `OD-RECOVERY-040` (OD-039 proved the full proven invocation inside the lease — gate verified 22:54:57Z, roll `2,090,084→…→13` in 40 rounds, gate held `OfflineReplayVerified` through the entire 120s lease and closed only at evidence expiry, the first full-lease survival since OD-031; the value-bound plateau (13, within the 11–17 range) was NOT staged because harvest only triggers at ≤10, so the driver now has a `-HarvestThreshold` param to stage the full converged tail for interactive Find-what-writes even without ≤10 — OD-040 runs the proven invocation WITH `-HarvestThreshold 200` and the operator present during the held green window; alternatively investigate the expiry-termination observation (managed child 53100 survived `EvidenceStale` ~7 min, gate stayed closed) or the replay-start flake root cause, or obtain a content-distinct second replay for BLK-0019 |
...
| `OD-RECOVERY-039` | 2026-08-03 | Full-lease green roll with the proven invocation; close the plateau-staging gap | background launch helper + chained relaunch orchestrator (5 attempts) + pre-arm-debugger.ps1 + proven invocation `-SnapshotMaxBytes 402653184 -MaxRounds 40` + **`-HarvestThreshold` tail-staging fix** + CE autorun (300s poll) | `Partial` | **Full green lease roll**: gate verified 22:54:57Z, roll **2,090,084→…→13 in 40 rounds** (`rolling_exit=0`, full round budget), **gate held `OfflineReplayVerified` through the entire 120s lease and closed only at evidence expiry 22:56:56Z** — first full-lease survival since OD-031; plateau **13** (value-bound, within the 11–17 range); CE staged 1 survivor + armed 1 write-BP, 0 hits (OD-020 negative consistent); attempts 1–4 verified-then-died at replay start (flake 4 of 5); **`-HarvestThreshold` fix added** — the plateau set was NOT staged because harvest only triggers at ≤10, so the operator window had 1 candidate instead of 13; observation: managed child 53100 survived `EvidenceStale` (~7 min past expiry) — expiry-termination property gap to check in OD-040 | No ≤10 target staged this run (plateau 13); interactive Find-what-writes still operator-owned; `independentReplays` still 0; no RIP/root |
...
## `OD-RECOVERY-039` result — 2026-08-03 (full-lease green roll; plateau-staging fix)

```yaml
sessionId: OD-RECOVERY-039
date: 2026-08-03
observedAtUtc: 2026-08-03T22:57:00Z # approximate; five attempts 22:37-22:57Z

timebox: run the proven invocation (-SnapshotMaxBytes 402653184 -MaxRounds 40 -HoldAfterRollSeconds 240) with the operator present during the held green window; the 11-survivor set is usable for interactive Find-what-writes even without <=10
decision: the FULL 40-round budget invocation completed inside the 120s research lease with the game surviving the ENTIRE evidence lifetime (first time since OD-031); the roll converged to the value-bound plateau at 13 (3 above target), the gate held OfflineReplayVerified through the whole roll and closed only at evidence expiry - not a flake. Durable finding: the plateau set is NOT staged because the driver harvests only at/below the <=10 target, so the operator window had 1 candidate instead of 13 - fixed with a new -HarvestThreshold driver param (stages the full converged tail before discard).
objective: per OD-039 protocol, run the proven invocation with the operator window held; capture the best roll the elevated flake rate allows; close the plateau-staging gap so the next operator window has the real survivor set
stopCondition: stop after a full green roll + operator hold, or after the flake rate exhausts the attempt budget
method:
  primaryTool: scripts/launch-offline-replay-for-od.ps1 (detached) + chained relaunch orchestrator + pre-arm-debugger.ps1 + roll-replay-time-increased.ps1 (proven invocation) + od-018 session driver
  secondaryTools: Cheat Engine 7.7 (C:\Program Files\Cheat Engine) pre-armed; CE autorun od-autorun-writebp.lua (300s poll)
  transition: natural replay progression between rounds (no manual Space pulses)
observations:
  - state: attempts-1-4
    outcome: launcher reached OfflineReplayVerified (watch_exit=0, round-2 click) then the game quietly exited ~2s after LoadGameScene ends (become hidden + GameCore::OnBackground) - the elevated replay-start flake, 4 of 5 attempts this session
    blitzLog: blitz-logs_20260803174515 (22:45:41 Start replay event -> LoadGameScene begins/ends 22:45:42/43 -> become hidden + OnBackground 22:45:45)
    note: attempt 3's session driver missed the brief verified window (5s poll vs ~3s game survival)
  - state: attempt-5 (full green lease roll)
    verificationState: OfflineReplayVerified (watch_exit=0, round-2 click 22:54:09...22:54:57Z chain attempt 2)
    rolling: sequence 2,090,084->50,112->10,364->2,512->239->66->48->32->29->26->23->18->17->15->14->13 (40 rounds, full round budget, rolling_exit=0); round-1 previous=2,090,084 (budget-bound, smaller than OD-038's 39M - footprint variance)
    gate: held OfflineReplayVerified through the entire 120s lease (verified 22:54:57Z, evidence expiry 22:56:56Z) - closed by lease expiry, NOT a flake; operator window held and closed at EvidenceStale
    staging: address file wrote only 1 candidate (0x74A2478) with WARN_address_count_mismatch candidates=1 survivors=13 - the target-path harvest never runs at >10, so the plateau set was NOT staged
    ceAutorun: attached pid 53100, staged 1 survivor, armed 1 write-BP, 20s capture, hits=0 (consistent with the OD-020 automated-write-BP negative)
  - state: driver-hardening
    change: added -HarvestThreshold param to roll-replay-time-increased.ps1 - when the roll ends without reaching the target but survivors <= HarvestThreshold, one final 500-candidate compare stages the full converged tail before session discard; passthrough added to od-018-session.ps1
    parseCheck: parse_ok (both scripts)
  - state: expiry-termination observation
    managedChild: wotblitz 53100 remained alive after EvidenceStale (last observed 23:03Z, ~7 min past expiry) despite the documented exact-managed-child termination at evidence expiry; host log shows only lifecycle_evidence verified, no termination line
    note: gate stayed closed (EvidenceStale = no scanning authorized); this is a cleanup/property gap to check in OD-040, not an authorization gap
result:
  whatWorked:
    - FULL green lease roll: the proven invocation (-SnapshotMaxBytes 402653184 -MaxRounds 40) completed all 40 rounds inside the 120s lease with the game surviving the entire evidence lifetime - the first time since OD-031 a roll ran to its full round budget and the gate closed only at lease expiry.
    - Gate held OfflineReplayVerified through the whole roll; the operator window closed at evidence expiry, not gate loss.
    - Value-bound plateau reproduced (13, within the OD-032/033 11-17 convergence range).
    - Plateau-staging gap root-caused and fixed: -HarvestThreshold stages the full converged tail so the next operator window has the real survivor set even without <=10.
  whatFailed:
    - Target <=10 not reached (plateau 13, 3 above target); only 1 of 13 survivors staged pre-fix.
    - Replay-start flake remains the binding constraint: 4 of 5 attempts verified-then-died this session.
    - Managed child survived evidence expiry (expiry-termination observation, unconfirmed cause).
  rulesOut:
    - Nothing new: the automated CE write-BP path stays ruled out (1 armed, 0 hits).
  partials:
    - Full-lease green roll with the proven invocation (pipeline end-to-end at best-known state).
    - -HarvestThreshold plateau-staging fix (parse-checked; pending live validation in OD-040).
    - BLK-0019 still needs a content-distinct second replay (independentReplays 0).
  nextPivot: OD-RECOVERY-040 - run the proven invocation WITH -HarvestThreshold (e.g. 200) so the converged plateau set (11-17) is staged for the operator's interactive Find-what-writes; alternatively investigate the expiry-termination observation (managed child survived EvidenceStale) or the replay-start flake root cause; a content-distinct second replay remains the promotion prerequisite (BLK-0019).
  repeatWithoutChangedHypothesis: false
artifacts:
  rawFiles: /tmp/od-launch-039{,b,c,d}.log, /tmp/od-session-039.log, /tmp/od-roll-039.log, blitz-logs_20260803174515.txt, %TEMP%\od-ce-autorun.log, %TEMP%\od-survivors.txt, %TEMP%\od-prearmed-debugger.json
  committedSummary: this ledger entry + workflow update + handoff (2026-08-03-od-recovery-039.md) + roll-replay-time-increased.ps1 -HarvestThreshold + od-018-session.ps1 passthrough
```

`OD-RECOVERY-039` is aggregate structural evidence only. Offset remains 0.
The session proved the full proven invocation inside the lease (first
full-lease green roll since OD-031) and closed the plateau-staging gap with
the `-HarvestThreshold` driver fix. The binding constraints remain the
replay-start flake (4 of 5 attempts) and BLK-0019 (single replay).
