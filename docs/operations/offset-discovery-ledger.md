...
| Next planned session | **CAM-010 follow-up — find the true render eye + orientation convention** (CAM-010 DONE 2026-08-11: GameCamera posA `+0x38` is stored (x, z, y) — world Y/Z SWAPPED; yz-swap puts it 2.1–3.6 m from the decoded viewpoint tank on all rounds of v7b `019ff26b` + v7c `019ff276` (sub-meter), and CAM-004's "23.57 m third-person offset" was the `√2·|tank.z − tank.y|` artifact (z−y = 16.7 m at the read moment), NOT a chase eye — CAM-004 verdict SUPERSEDED, validator `eye` now yz-swaps posA, camera NOT yaw-locked to the tank (101–177° gaps). Open: the orientation convention and the true render eye (candidate: the ReplayCameraController `+0x28` per-frame ring — dump controller + ring region on one launch, any phase, and search for a non-tank triple consistent with a chase eye / the actual render view; `-CaptureWindow` scalars stay diagnostic-only). W2S seam must yz-swap world→camera space.) **OD-RECOVERY-090 is DONE (2026-08-11): HONEST-NEGATIVE** — no damage-dealt counter in the 320-byte entity-base region (session `019ff250`, 50 dumps, all 6 event windows + controls; top candidate `+0x3C` score 0.833 demoted by flatness 0.091 — it is the already-measured position-copy float; damage totals corrected to 6/752). Re-attempt needs a wider region / sibling-record sweep. OD-RECOVERY-089 is DONE (2026-08-11): **L2 facing Phase-4 HIT** — ring-record `+0x30` agrees on Dead Rail (56/56, score 1.0, flatness 1.0), `twoReplayRepeatability = true`; the at-session honest-negative was a matcher limitation (one-directional shared lag vs the opposite-sign per-dump G2 label skew), fixed additively by the per-dump bidirectional lag path and proven on both replays. The yaw publication package is READY (`docs/operations/g1-yaw-publication-draft.md` — operator approval + gate run only); the Phase-4 two-replay HP rule (Dead Rail victim 2549399) still gates HP publication; item 7 (hardware atomicity) stays LAST. |
