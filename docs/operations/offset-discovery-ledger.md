...
Last updated: 2026-08-03 (OD-042-STATIC: **`0x037F3054` precisely identified as the shared RTTI `type_info` vftable** — every TypeDescriptor's pVFTable points at it (confirmed td@0x03DB5120 `pVFTable == 0x037F3054`), explaining the 48,609 `.data` references; the constructor at 0x020CB956 writes it to `[this]+0` (canonical vftable init); **`--vtables` now names 17,133 of 18,721 vtables** via the COL chain — the fix: MSVC x86 stores the mangled name **inline at td+8** (char[]), not as a VA pointer; chain-class vtables located incl. `GameScene` 0x0319D3C4 (26 slots, **0 .data roots** = honest negative for the vtable-singleton path), `BaseContext`/`RootContext`, Vehicle component family; **Vehicle-family TypeDescriptor xref = 0 .text refs / 0 slots** — RTTI name→root path exhausted; prior milestone OD-041-STATIC: the two 'root candidates' reclassified as members of a repeating 0x50-byte record family, NOT gameplay roots; rolling driver gained delta pass-through; prior session OD-RECOVERY-038: lobby-login red herring corrected; 401-refresh hardening validated; roll 39M→11, plateau 11 = value-bound)
...
| Next planned session | `OD-RECOVERY-043` (static milestones `OD-041/042-STATIC` complete: **`0x037F3054` is the shared RTTI `type_info` vftable** — every TypeDescriptor's pVFTable points at it (confirmed td@0x03DB5120), which is why 48,609 `.data` slots reference it; `0x03FA0C74` remains a member of the repeating 0x50-byte record family, NOT a gameplay root; **`--vtables` now names 17,133 of 18,721 vtables** via the COL chain (MSVC x86 stores the mangled name **inline** at td+8, not as a pointer — this was the fix) including `GameScene` 0x0319D3C4 (26 slots, **0 .data roots** = honest negative for vtable-singleton path), `BaseContext`/`RootContext`, and the Vehicle component family; Vehicle-family TypeDescriptor xref = **0 .text refs / 0 slots** — the RTTI name→root path is exhausted for the chain classes). Highest-value live run: the proven invocation `-SnapshotMaxBytes 402653184 -MaxRounds 40 -HoldAfterRollSeconds 240` with the operator present during the held green window — the 11-survivor replayTime set is usable for interactive Find-what-writes even without ≤10 — optionally with a **delta-compare pilot** feeding a replay-derived position delta to break the plateau; alternatively a content-distinct second replay for BLK-0019, or investigating the replay-start flake root cause (the game dies quietly ~2s after `LoadGameScene` ends with no crash dump) |
