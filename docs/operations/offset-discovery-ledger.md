...
immutable OD-088/089 dumps re-verdict to HIT per member: savanna 48/48 and
medvedkovo 56/56 at `+0x2C` (pitch) and `+0x28` (roll), each score 1.0 /
flatness 1.0; `twoReplayRepeatability = true` for the FULL rotation triple.
**Methodology lesson (ring-sibling decoy):** the savanna roll verdict on the
...
verdicts in ONE command: savanna roll `+0x28` 48/48 (the former `0x60` decoy
is gone), pitch `+0x2C` 48/48, yaw `+0x30` 48/48; medvedkovo roll `+0x28`
...
medvedkovo** — ring-record `+0x30` agrees (56/56, score 1.0, flatness 1.0;
...
per-dump variable and OPPOSITE in sign between replays (savanna memory lags
+4.8 s; medvedkovo memory leads −2.5 s). The additive per-dump bounded
...
Phase-4 repeat CLOSED HIT on medvedkovo** — `[entity+0xB8]` current-health
signed int16 agrees with savanna (score 1.0, flatness 1.0, Strict 4/4 exact
sums after the additive lead-side attribution window
`--lag-lead-seconds`; `twoReplayRepeatability = true`). The at-session
verdict was honest-negative: medvedkovo's memory clock LEADS the decoded
...
re-verdicts the SAME immutable dumps to HIT, and the savanna dumps re-verify
...
and exactly one unchanged bounded poll on that replay (savanna) returned
`stable-resolver-positive` 24/24 — proving **cross-replay continuous-polling
repeatability across two distinct 11.19.0 replays** (medvedkovo + savanna;
...
| Runtime-supported fields | **8 fields `Verified` via module-rooted `chains`** (offsets stay 0 by design): `playerPositionX/Y/Z` (position-ring chain, OD-RECOVERY-083/084), `playerHP` (entity-base chain `[entity+0xB8]`, OD-RECOVERY-087/091), `playerYaw` (ring-record chain `+0x30`, OD-RECOVERY-088/089) — G1 applies 2026-08-12 OD-RECOVERY-092; **rotation triple ready for publication (2026-08-12 reconciliation): roll `+0x28` / pitch `+0x2C` agree on both replays (48/48 savanna + 56/56 medvedkovo, `--field roll|pitch` on the same immutable dumps)** — the pitch/roll chains mirror yaw (position hops 1..11 + recordOffset 0x2C / 0x28); the rest Unknown |
| `playerYaw` | **Quarantined → resolved-by-supersession (2026-08-11)** — the address-kind question is answered: yaw is a runtime chain field on the movement ring record (**record `+0x30`, live-verified 2026-08-11 by OD-RECOVERY-088 on savanna AND OD-RECOVERY-089 on medvedkovo — `twoReplayRepeatability = true`**; the rehearsal's +0x2C prediction was self-constructed — the tail is roll `+0x28` / pitch `+0x2C` / yaw `+0x30`), not a static module offset; the legacy static candidates were mutually inconsistent and are retired. Publication READY via `docs/operations/g1-yaw-publication-draft.md` (operator approval + gate run only) |
...
`+0x2C`, yaw `+0x30`** (48 savanna region dumps, sub-0.1 deg alignment
...
| `OD-RECOVERY-076` | 2026-08-09 | Resolve BLK-0026 and prove cross-replay continuous polling | Launcher marker-ACL fix (`Set-Acl` → `icacls`) + exactly one unchanged bounded poll on the content-distinct replay | `Hit` / `Complete` (cross-replay continuous polling) | Root cause proven: .NET `Set-Acl` threw `PrivilegeNotHeldException` on the persisted owner-only marker ACL (every launch after the first); launcher reaches `OfflineReplayVerified` after the fix; unchanged bounded poll on the savanna replay resolved 24/24 with 24 distinct triples, 12 within one unit, and 21/24 within three | Cross-replay continuous polling proven across two distinct 11.19.0 replays (medvedkovo + savanna); still no hardware atomicity, same-clock proof, numeric-offset publication, or offset promotion |
...
method: scripts/python/replay-delta-extractor.py (new) + roll-replay-time-increased.ps1 -ValueKind (new) against the decoded 11.19.0 medvedkovo session
...
findings:
  ...
  - extractor: replay-delta-extractor.py
    session: 019fb86c-c8e7-7004-9df6-a574f5a7835b (11.19.0, medvedkovo, 33,281 position samples)
...
method: replay-delta-extractor.py --simulate (new mode) on the 11.19.0 medvedkovo session 019fb86c-… (4s window, 2,779 window measurements, most-moving participant)
...
date: 2026-08-03
observedAtUtc: 2026-08-03T15:20:00Z # approximate; both modes run against the decoded 11.19.0 medvedkovo session 019fb86c
...
method:
  ...
  inputSession: 019fb86c-c8e7-7004-9df6-a574f5a7835b (11.19.0 medvedkovo, 2,784.6s, 1e6 ticks/sec)
observations:
...
- **T1/T2 anchors** from the decoded 11.19.0 medvedkovo session
...
**Run.** One launch (medvedkovo content 59c3b92eb221 via 36f5ab..., replay
...
data: FRESH43 capture `od-048-autotrace-20260807-123621.json.capture.json` (6 hits);
      decoded session `019fb86c-c8e7-7004-9df6-a574f5a7835b` (sha 59c3b92e…, medvedkovo)
...
data: decoded session `019fb86c-c8e7-7004-9df6-a574f5a7835b` (sha 59c3b92e…, medvedkovo, 435k samples)
...
- 16 files: medvedkovo, battle `2026-07-29T17:35:16`, sha `59c3b92e…` — the
  SAME payload as FRESH43's medvedkovo replay (re-recorded by the offline
  viewer; identical 1 100 265-byte size).
- **2 files**: savanna (savanna), battle `2026-08-02T21:15:07`, sha
...
- gameVersion 11.19.0, map savanna (mapId 11), battle 2026-08-02T21:15:07,
...
crossReplayPoll:
  replay: content-distinct savanna 11.19.0 replay (1,045,525 B, battle 2026-08-02)
...
the medvedkovo replay, cross-replay continuous-polling repeatability is proven
...
same savanna replay: launcher to `OK OfflineReplayVerified`, position-page
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/invoke-g1-live-poll.ps1 `
  -ReplayPath <savanna-replay> -WindowWaitSeconds 240 -SkipInterceptorArm
```
...
evidence:
  ...
  priorAggregates:
    - .data/od-073-entity-position-poll-20260809-021445.json (OD-075, medvedkovo)
    - .data/od-073-entity-position-poll-20260809-165144.json (OD-076, savanna)
privacy:
  publicProcessAddressesOrRawBytes: false
  aggregatePersistsIdsOrCoordinates: false
  trackedPrivateArtifactValues: false
...
- The G0 review's own G3 definition is ledger attestation of two distinct
  replays with fresh processes (medvedkovo OD-075 + savanna OD-076) —
  satisfied, plus this run is a third fresh savanna process.
...
mode: approved live session on savanna (5 launches, 4 gated OK, 1
...
mode: approved live session on savanna (4 launches, 3 gated OK, 1
...
two-replay rule for HP (medvedkovo victim 2549399) still applies before any
HP publication.
...
proof:
  ...
  twoReplayRepeatability: false - pending medvedkovo agreement (Phase-4 rule)
  liveFrameYawBecomesLive: true (RingRecordRegion now decodes +0x30)
```

### Session summary

One approved live launch (savanna, launcher exit 0, gate
...
The facing rehearsal (27/27 savanna + 35/35 medvedkovo, both "HIT at +0x2C")
...
medvedkovo - the same offset must agree, exercising the 5 seam crossings)**,
then CAM-001 v7; the Phase-4 two-replay rule for HP (medvedkovo victim
...
status: Closed (L2 facing Phase-4 repeat on medvedkovo, HIT at +0x30)
mode: invoke-facing-session.ps1 -SessionId 019ff209-a4cb-7c2b-88e3-2b0eaf65c490
  ...
  on medvedkovo: launcher to OfflineReplayVerified, then
  /discover/entity-region ring-record dumps (region 256, replay-clock
  labeled, sameDecodedClockProven required) per turn segment + flat
  controls -> yaw-diff verdict (value-match lag path, wrap-aware)
targetBuild:
...
liveRun:
  ...
  verdict: HIT at +0x30
proof:
  twoReplayRepeatability: true (yaw +0x30 on savanna 088 AND medvedkovo 089)
  ...
  labelSkewFinding: per-dump replay-clock label skew, opposite sign per
    replay (savanna memory lags +4.8 s; medvedkovo memory leads -2.5 s),
    spread 5.6 s (medvedkovo) / 13.1 s (savanna)
```

### Session summary

One approved live launch (medvedkovo, launcher exit 0, gate
`OK OfflineReplayVerified`, clock anchored at the blitz-log marker moment,
...
against the decoded packet yaw on medvedkovo, but the G2 replay-clock LABEL
skew is OPPOSITE in sign vs savanna: savanna memory LAGS the label (+3..5 s),
medvedkovo memory LEADS it (-2..5 s), and it varies per dump. The
one-directional shared lag path (memory-behind-packet only) structurally
cannot see the medvedkovo sign; the shared bidirectional path hits 56/56
...
|---|---|---|---|---|---|---|---|
| medvedkovo 089 | per-dump bidirectional | +0x30 | 1.0 | 1.0 | 56/56 | -2.5 s | 5.6 s |
| savanna 088 (re-run) | per-dump bidirectional | +0x30 | 1.0 | 1.0 | 48/48 | +4.8 s | 13.1 s |
...
**OD-RECOVERY-090 (L3 damage-dealt, savanna attacker 3760577)** + its Dead
Rail Phase-4 repeat (attacker 2549401); the Phase-4 two-replay rule for HP
(medvedkovo victim 2549399) still applies before any HP publication; item 7
...
- **Re-attempt (if any):** widen the region (batch surface cap ~0x1000)
  ...
  exact-sum ≥ 2, flatness 1.0). medvedkovo repeat (attacker 2549401) stays
  the Phase-4 gate for any future hit.
- **Gate position:** HP publication keeps its own Phase-4 rule (medvedkovo
...
work: HP Phase-4 (medvedkovo victim 2549399) for HP publication.

## OD-RECOVERY-091 result — 2026-08-11 (L1 HP Phase-4 repeat — CLOSED HIT at entity-base +0xB8; `twoReplayRepeatability = true`)

status: Closed (HP Phase-4 two-replay repeat on medvedkovo, HIT at +0xB8)

**Session.** Four approved live launches on the content-distinct medvedkovo
...
medvedkovo G2 label skew: the memory clock LEADS the decoded clock by ~2.5 s
...
measured medvedkovo lead + margin). Three new correlator tests: lead exact
...
savanna regression on the same parameters (`hp-snapshots4.json`, session
...
OD-RECOVERY-087 on savanna). The Phase-4 two-replay HP rule CLOSES;
...
**Camera Branch B — closed.** medvedkovo session
`01a0021f-e7f3-7434-9286-d9ea0a3eaca1` and savanna session
...
**Batch read-pass timing — measured on two replays.** medvedkovo persisted
...
savanna's first 20/90/180 schedule lost rendezvous transiently before its third
...
**Adjacent cluster closures.** medvedkovo's live frame at 158.9 s carried a
...
HIT verdicts (score 1.0, flatness 1.0). The medvedkovo same-replay rerun then
...
## OD-RECOVERY-102 result — 2026-08-16 (savanna batch position re-verdict)
...
stored savanna batch positions from the item-7 live cluster with a bounded
...
is the decoded savanna session `019fee20-9315-70b7-a92c-379f41f69532`
...
same savanna memory-apply lag the yaw lane measured (+4.8 s median,
