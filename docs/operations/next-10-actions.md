...
|---|---|---|---|---|
| 1 | **Derive and live-validate the viewpoint-vehicle → `VehicleGun`/`VehicleGunRotator` ownership walk** | Phase 6 v0.3 G1/G2; BLK-0027 | The live census found 42 `VehicleGun`, 1 `VehicleGunRotator`, 1 `AvatarGunAgent` (stable) in two content-distinct battles (Oasis Palms + Dead Rail) — the single rotator/agent pair is the candidate viewpoint owner; the walk must pin which gun/rotator the viewpoint vehicle owns before field offsets are trusted | offline producer evidence; one exact-build launch to validate |
