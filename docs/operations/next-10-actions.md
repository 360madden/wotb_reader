...
|---|---|---|---|---|
| 1 | **Repeat the semantic-field live snapshot on a second content-distinct replay**, then isolate gun elevation and join a decoded shot | Phase 6 v0.3 G1 items 2/5; BLK-0027 | First replay is live-positive (`2026-08-16-pen-semantic-fields-live.md`): 56/56 walk+marker+reload snapshots, marker yaw moved across 4 bins while hull stayed in 1 bin (8 independent windows). Reload enum changed (`0`/`3`/`9`) so it is not a shell. Elevation, shot join, and the second replay remain | one more approved `OfflineReplayVerified` launch; no promotion from one replay |
