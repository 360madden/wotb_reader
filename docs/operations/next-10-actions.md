...
|---|---|---|---|---|
| 1 | **Live-validate the viewpoint-vehicle → `VehicleGun`/`VehicleGunRotator` ownership walk** (static half done; proposal drafted) | Phase 6 v0.3 G1/G2; BLK-0027 | Static hash-bound derivation is done (`pen-ownership-walk-proof-protocol.md`): `AvatarGameLogic +0x1fc → VehicleGunRotator` and `+0x204 → VehicleGun`, with `rotator+0x10 → owner` and `+0x04 → entity`. The five-read protocol must confirm H1 live; the shared-contract proposal is drafted (`pen-ownership-walk-live-validation-proposal.md`, additive `PenOwnershipWalk` anchor) and awaits owner decision + the read-only security audit | owner approval + security audit; then one exact-build launch |
