...
|---|---|---|---|---|
| 1 | **Static-follow `FUN_0133a410` / rotator `+0xEC` for the GetGunMarkerPosition start pose.** Do not live-read an unproven matrix slot | Phase 6 v0.3 G1 item 5; BLK-0027 | Published `+0x50` pos is a far hit. Reconstructing start as `hit-dir*(scalar/(2*param3))` is a live no-go (0/24; scalar is not a long range) | no speculative live matrix read; do not promote |
