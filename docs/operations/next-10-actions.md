...
|---|---|---|---|---|
| 1 | **Repeat elevation isolation on a second replay and make shot-join shot-synchronous** (faster than AOB-per-sample, or a capture window that covers viewpoint ShotImpact times) | Phase 6 v0.3 G1 items 2/5; BLK-0027 | First elevation-independent windows are live (5 on `01a00d08`; pitch 8 bins, hull 1 bin). Shot join saw 5 viewpoint shots and 0 joins: all `lagExceeded` even at 10 s. Two-replay marker-yaw independence already stands | do not promote; do not treat 10 s lag as shot-sync |
