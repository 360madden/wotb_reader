...
| 8 | **Pen-chance HUD (PN) — PN-4 validation loop next** (score the model vs decoded type-8/type-32 shot outcomes). PN-1/2/3 DONE: armor/shell/gun parsers + install data service, the `.scg` collision-mesh parser + raycast (`CollisionMeshParser`/`CollisionRaycast`, true surface normal), pure pen math, and the reticle-centered badge wired end-to-end (frame → response → WPF HUD). Honest limits: front-only armor, stock AP shell, thickness still nominal | Phase 6 (`docs/operations/pen-chance-design.md`) | The remaining proof is scoring, gated on: (a) a per-shot ATTACKER AIM (decoded or the live camera — the center-line proxy was shown too coarse), and (b) pinning the type-32 damage-vs-bounce payload field (the mirror is ALREADY in the store — 66 386 raw records, flag prefixes mapped 2026-08-13; the damage/bounce discriminator is in the payload, not the prefix) | offline; PN-4 no launch |
