...
| 8 | **Pen-chance HUD (PN) — PN-4 validation loop next** (score the model vs decoded shot outcomes). PN-1/2/3 DONE: armor/shell/gun parsers + install data service, the `.scg` collision-mesh parser + raycast, pure pen math (normalizationAngle + piercingPower drop both modeled), the reticle-centered badge wired end-to-end, and the type-32 mirror DECODED into `ShotImpact` (hit-result byte pinned ~98% on three replays). Attacker attribution is PARTIAL: type-8 subtype-1 damage = pens (100%), type-8 subtype-8 (attacker at +0x0C) = some bounces. **Thickness per face stays blocked** (probed 2026-08-13): the `.scg` is one merged `PolygonGroup`, `primaryArmor` lists the frontal ARC not a clean face split, and the per-part `hitTester` models are packed — so side/rear/turret stay fail-closed Unknown | Phase 6 (`docs/operations/pen-chance-design.md`) | The remaining proof is scoring — a C# geometry harness (reuse `PenetrationDataService` + `CollisionRaycast`) that raycasts each attributable shot's center-line aim against the victim mesh and checks the ricochet threshold vs the `ShotImpact` ground truth; the store's `tank_id`/`tank_name` columns are inconsistent, so descriptor→tank-id must go through the enrichment, not raw SQL | offline; PN-4 no launch |
