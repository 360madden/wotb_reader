...
| 8 | **Pen-chance HUD (PN) — PN-4 validation loop next** (score the model vs decoded shot outcomes). PN-1/2/3 DONE: armor/shell/gun parsers + install data service, the `.scg` collision-mesh parser + raycast (`CollisionMeshParser`/`CollisionRaycast`, true surface normal), pure pen math, the reticle-centered badge wired end-to-end (frame → response → WPF HUD), and the type-32 mirror DECODED into `ShotImpact` (hit-result byte pinned: `0x03` = pen, `0x00–0x02/0x04` = bounce/absorb, ~98% on three replays). Honest limits: front-only armor, stock AP shell, thickness still nominal | Phase 6 (`docs/operations/pen-chance-design.md`) | The remaining proof is scoring, gated on ONE prerequisite: a per-shot ATTACKER AIM (decoded attacker→victim direction at the shot instant, or the live camera) — the center-line proxy was shown too coarse; the pen/bounce ground truth itself is now available per shot | offline; PN-4 no launch |
