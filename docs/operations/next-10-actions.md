...
|---|---|---|---|---|
| 1 | **Live-validate the viewpoint-vehicle → `VehicleGun`/`VehicleGunRotator` ownership walk** (anchor implemented; live run pending) | Phase 6 v0.3 G1/G2; BLK-0027 | Static chain derived (`pen-ownership-walk-proof-protocol.md`): `AvatarGameLogic +0x1fc → VehicleGunRotator`, `+0x204 → VehicleGun`, `rotator+0x10 → owner`, `+0x04 → entity`. The additive `PenOwnershipWalk` entity-region anchor is implemented, unit/endpoint-tested (9+1), and lead-audited (aggregate-only, fail-closed, rotator identity re-read) — what remains is running it on one exact-build managed offline replay to confirm H1 live | one exact-build managed offline launch (`OfflineReplayVerified`) |
