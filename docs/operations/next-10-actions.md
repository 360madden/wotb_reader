...
| 6 | **Pen-chance HUD (PN) — geometry harness + PN-4 validation next** (PN-4 offline is BLOCKED: the position stream has no projectile entities, so per-shot shell direction is unavailable; PN-4 needs live CAM-013 aim). PN-1/2/3 DONE: armor/shell/gun parsers + install data service, the `.scg` collision-mesh parser + raycast (mesh Z-up orientation + steep-glacis face classification both fixed), **per-part armor LANDED 2026-08-13** (hull/turret/gun union raycast + turret frontal `primaryArmor`), **shell kind + manual selector LANDED 2026-08-13** — `ShellSpec.Kind` excludes HEAT from the 3-caliber overmatch rule, and the viewer can pick any shot of its stock gun (AP/APCR/HE/HEAT) via the **sidebar ComboBox** or the <c>Q</c> hotkey (`?shell=` → `PenShells`/`PenShell` on the frame); pure pen math (ricochet on the RAW angle, two-caliber, ±5% RNG, HE never-ricochet); badge wired end-to-end (struck face FRONT/SIDE/REAR shown, anchored to the aimed tank's nameplate with a reticle fallback); type-32 mirror DECODED into `ShotImpact` (~98% on three replays). **The PN-4 scoring harness core SHIPPED 2026-08-13** (`PenValidation.Score` + `ScoredShot`, 6 tests — ricochet + band agreement vs the decoded `penetrated` outcome). **The aim-source WIRING SHIPPED 2026-08-14:** `PenOfflineScorer` + `GET /discover/pen-offline-score/{sessionId}` drive the real `PenValidation.Score` over the decoded 69-shot session (per-tank `ResolveTankAsync` lane incl. raw-compact-descriptor resolution through the installed-game index) — first result 18 classified / 7 agree (38.9%), 46 Unknown (side-armor nominal-0), all 6 predicted ricochets actually penetrated: the center-line + stock-shell proxy quantified honestly. Remaining: the live CAM-013 capture replacing only the aim source. `primaryArmor` lists the frontal ARC so per-group side/rear thickness stays unknown (fail-closed) | Phase 6 (`docs/operations/pen-chance-design.md`) | PN-4 live validation (CAM-013 aim at shot time vs `ShotImpact`) is the proof; the scorer is the reusable core. **Live pen badge WIRED 2026-08-13:** `GetLiveFrameAsync` now resolves the install penetration context and `LiveFrameProjector` scores the CAM-013 chase-camera aim, so the badge renders in LIVE mode too (fail-closed to null without the data; `?shell=` threads the selector through the live frame). **Aim scoping COMPLETE 2026-08-13:** both paths exclude the own tank, same-team allies (enemy-only), and destroyed wrecks (alive-only) — unknown team/alive stays eligible (fail-open) | offline; PN-4 1 launch |
