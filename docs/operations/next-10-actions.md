...
|---|---|---|---|---|
| 1 | **Live-validate the viewpoint-vehicle → `VehicleGun`/`VehicleGunRotator` ownership walk** (static half done) | Phase 6 v0.3 G1/G2; BLK-0027 | Static hash-bound derivation is done (`pen-ownership-walk-proof-protocol.md`): `AvatarGameLogic +0x1fc → VehicleGunRotator` and `+0x204 → VehicleGun`, with `rotator+0x10 → owner` and `+0x04 → entity`. The five-read gated protocol must now confirm H1 live on one exact-build launch before shell/aim/ray offsets are trusted | one exact-build launch to validate the protocol |
