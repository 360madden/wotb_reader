...
| 8 | **Pen-chance HUD (PN) — shell selector + PN-4 validation next** (PN-4 offline is BLOCKED: the position stream has no projectile entities, so per-shot shell direction is unavailable; PN-4 needs live CAM-013 aim). PN-1/2/3 DONE: armor/shell/gun parsers + install data service, the `.scg` collision-mesh parser + raycast (mesh Z-up orientation + steep-glacis face classification both fixed), **per-part armor LANDED 2026-08-13** — `CollisionMeshParser.ParseAll` reads all three groups (`#id` 1/3/5 = hull/turret/gun) and `SceneFileParser` walks the `.sc2` SFV2 v2 value stream to VERIFY the parts' identity transforms (one shared rest-pose space → union raycast, turret/gun scored against the turret's frontal `primaryArmor`); pure pen math (ricochet on the RAW angle, two-caliber, ±5% RNG, HE never-ricochet); reticle badge wired end-to-end; type-32 mirror DECODED into `ShotImpact` (~98% on three replays). Remaining offline: (a) a manual shell selector (AP/APCR/HE/HEAT) to surface the already-modeled shell-aware ricochet/normalization, and (b) the C# geometry harness that scores the ricochet rule vs `ShotImpact` once a real aim source exists. `primaryArmor` lists the frontal ARC so per-group side/rear thickness stays unknown (fail-closed) | Phase 6 (`docs/operations/pen-chance-design.md`) | PN-4 live validation (CAM-013 aim at shot time vs `ShotImpact`) is the proof; the harness + shell selector are offline pre-work | offline; PN-4 1 launch |
