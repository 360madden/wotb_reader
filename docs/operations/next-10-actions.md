...
| 8 | **Pen-chance HUD (PN) — PN-3 replay HUD next** (aim = camera pose CAM-013 → pen badge on the aimed nameplate), then PN-4 validation loop. PN-1 PROBED (armor/shell/gun data present + readable) and PN-2 DONE (`Core/Overlay/ArmorPenetration.cs`) | Phase 6 (`docs/operations/pen-chance-design.md`) | The long pole (static data) is resolved — the remaining work is wiring + scoring the model against decoded type-8/type-32 shot outcomes; plate-slope `.model` geometry is the open sub-problem | offline; PN-3 no launch |
