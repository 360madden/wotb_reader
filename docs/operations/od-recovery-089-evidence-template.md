# OD-RECOVERY-089 live-run evidence — L2 facing Phase-4 repeat on medvedkovo (COMPLETE)

**Status: COMPLETE (2026-08-11) — `twoReplayRepeatability = true` for yaw.**

The Phase-4 rule requires the yaw offset found live on savanna
(OD-RECOVERY-088: ring-record `+0x30`) to **agree on a second,
content-distinct replay** — medvedkovo, whose 5 seam crossings exercise the
wrap-aware matcher (yaw Δ p90 48.5° vs savanna 24.4°).
...
- **QUALIFY (offline):** medvedkovo replay; target entity **2549408**,
...
- **VERDICT (corrected, offline on the SAME immutable dumps):** root cause
  ...
  replay-clock LABEL skew is **opposite in sign per replay** (savanna: memory
  LAGS the label ~3–5 s; medvedkovo: memory LEADS ~2–5 s) and **per-dump
  variable**. The one-directional shared search cannot see the medvedkovo
...
| medvedkovo 089 | shared bidirectional (no per-dump) | `+0x30` | 1.0 | 1.0 | 56/56 | −2.5 s | — |

The shared-bidirectional path also hits on medvedkovo because the stationary
...
- A zero/constant-filled field cannot fake this: the fixture discipline
  ...
  7.8 % (savanna) / 14.1 % (medvedkovo) zero-filled float32 slots in the raw
...
status: CLOSED
replay: medvedkovo (content-distinct, 5 seam crossings)
verdict: Hit at +0x30
score: 1.0
matchedDumps: 56/56
...
- The Phase-4 two-replay HP rule (medvedkovo victim 2549399, `hp-diff`
