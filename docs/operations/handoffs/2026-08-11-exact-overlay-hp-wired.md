...
While verifying the overlay frame on medvedkovo, `tr00per_2015` (2549397)
...
**Caught 3 tanks the position markers missed**: medvedkovo 2549397 (@183.8s)
and 2549402 (@271.5s), savanna 3760576 (@245.1s). Alive flags now match the HP
...
| Check | Result |
|---|---|
| HP conservation | **28/28 tanks** on both replays: `current = max − taken` is never negative (destroyed tanks land at exactly 0, survivors keep positive HP — author 700/700, JOY 361/620, savanna victim 3760578 367/1550) |
| Ledger balance | total dealt == total taken exactly: medvedkovo 6227/6227, savanna 8964/8964 |
| Alive alignment | every ledger-dead tank has a Destroyed event and vice versa — 0 mismatches (9 destroyed medvedkovo + 9 destroyed savanna) |
...
both fresh sessions, exit 1 on the pre-fix medvedkovo session (21 errors:
...
- The scoreboard's `DamageDealt` uses the same corrected ledger, so the
  on-screen totals are now the true battle numbers (savanna 8964, not the old
  inflated 22094).
