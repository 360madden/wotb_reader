...
- Walked the **complete 11.19.0 packet-type inventory** on both replays (73 993
  packets on savanna) — the first 100% coverage of the stream, including
...
| Type | Count (savanna) | Semantics |
|---|---|---|
| 32 | 258 | **damage/impact event mirror** — same-instant as type-8 direct-damage (81/85 savanna, 107/120 medvedkovo; every miss is amt=0), embeds the SAME 6-byte shell signature (`a6 a5 e0 a2 a8 b1` @ t=69.13 vs type-8 at the same time) |
...
Counts are savanna-only; the earlier "unexplored" table (31/35/39) is now part
...
- Type-32/damage alignment: 81/85 savanna, 107/120 medvedkovo (misses are all
  amt=0/no-damage).
...
- HUD death check on live web host: 0/13–0/14 tanks ever `alive=false` on
  savanna (t=180/230/274) — confirms the gap on real data.
