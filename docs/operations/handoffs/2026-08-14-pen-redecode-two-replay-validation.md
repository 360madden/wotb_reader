...
Re-decoding Dead Rail also produced its first `ShotImpact` events (the old
...
|---|---|---|---|---|---|
| Oasis Palms `019ffdcd` | 67 | 2 | 18 | **38.9%** (7/18) | 0/6 |
| Dead Rail `01a00028-ddb0…` | 78 | 8 | 23 | **69.6%** (16/23) | 4/6 |

The spread is the documented offline limit in action: Oasis Palms is a
Churchill-vs-IS-7 match-up whose steep glacis hits the center-line proxy
misreads (predicted ricochets actually penetrated), while Dead Rail's varied
...
3. Add DLC-pack vehicle lists to `BuildManifestAsync` so premium tanks like
   `17425` enrich (also unblocks the 4 Dead Rail "victim data unavailable"
   skips).
