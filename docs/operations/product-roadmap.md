...
| L1 | D | HP live session (Oasis Palms victim 3760578; anchor **entity-base**, region ≥ 0x120, correlate **int16** — `VerifyPlayerHpChain` 2026-08-11 (16/16) pins the health block: current int16 `+0xB8`, alive byte `+0xBA`, **max int16 `+0x11C`**, healing int16 `+0x11E`, packed gun angles `+0x7E`; the `+0x48` from rehearsals is a SYNTHETIC FIXTURE, refuted; `hp-diff --int16 true` is the HP default; max HP at `+0x11C` also feeds the overlay HP fraction) | 1 |
