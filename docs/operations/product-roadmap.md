...
| P2 | H | HP publication — **READY (2026-08-11)**: 2-replay live agreement at entity-base `+0xB8` signed int16 (Oasis 087 Strict 8/8 + Dead Rail 091 Phase-4 Strict 4/4, `twoReplayRepeatability = true`); package `g1-hp-publication-draft.md`, operator approval + gate run only |
| P3 | H | Facing/yaw publication — **READY (2026-08-11)**: ring-record `+0x30` 2-replay agreement (Oasis 088 48/48 + Dead Rail 089 Phase-4 56/56, `twoReplayRepeatability = true`), replacing the quarantined candidate; package `g1-yaw-publication-draft.md`, operator approval + gate run only |
| P4 | H | Pitch/roll publication (if the ring record holds them) — the rotation triple is live-verified (`+0x28` roll / `+0x2C` pitch / `+0x30` yaw, OD-RECOVERY-088) but pitch/roll have NO Phase-4 repeat; P4 candidate only |
