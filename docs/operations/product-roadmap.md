...
| O5 | E | ✅ `--heading-delta` extractor mode (movement-gated, wrap-aware): motion-heading + packet-yaw per-window deltas, seam-crossing count (5 on medvedkovo, 0 on savanna), recommended live pilot target; `pick_yaw_session` selects the yaw-bearing decode |
| O6 | E | ✅ **Max HP is IN the replay — type-5 spawn full-state broadcast** (2026-08-11): `u32 eid @ +0x00`, `u16 currentHP @ +0x33`; the first broadcast per roster entity precedes any damage (28/28 tanks both replays), so it equals max HP. Validated: author 700 == battle_results `hitpoints_left` (both replays), monotonic non-increasing across broadcasts, total damage_dealt ≤ Σ first values (8964 ≤ 12140 / 6227 ≤ 8500), same tank_id same value cross-replay. Decoder now emits `CanonicalEventKind.MaxHealthObserved` per roster entity (first broadcast only; non-roster 999-style entities filtered) + typed raw record. Real-replay check: 14/14 savanna + 14/14 medvedkovo exact. This gives the overlay a true HP-fraction denominator without any memory read — ✅ **Damage amount SOLVED (2026-08-11)**: the real ledger is type-8 **subtype-1** (19 B): victim u32 +0x00, post-hit HP u16 +0x0C, attacker i32 +0x0E; amount = victim HP delta seeded by the type-5 max-HP broadcast; post-hit HP 0xFFFD is the destroy marker with killer attribution (remaining HP credited to the killer). `TryReadDirectDamage` (old subtype-8 amount) was WRONG — it inflated damage ~2.5–7× and dropped the real packets. Re-decode validation: per-attacker sums == battle_results damage_dealt EXACTLY on both replays for every player with battle results (9/9: 1719/1158/56/959/752 savanna + 1598/326/489/905 medvedkovo); players without battle results (left the battle, NULL stats) still get true decoded damage. ✅ **Exact overlay HP wired (2026-08-11)**: `ReplayFrameSource` now computes `hpFraction = 1 − taken/maxHealth` (falls back to the damage arc when max is unknown), and `OverlayTankState`/API/HUD carry exact `MaxHealth`/`CurrentHealth` — the nameplate shows "438 / 700" under the HP bar. Verified on both fresh imports: HP conservation 28/28 tanks (current = max − taken never negative), total dealt == total taken (6227/6227, 8964/8964). Along the way the subtype-1 0xFFFD destroy marker now ALSO emits `Destroyed` (deduped with the position marker): it caught **3 tanks the position markers missed** (medvedkovo 2549397 @183.8s, 2549402 @271.5s; savanna 3760576 @245.1s) — alive flags now match the HP ledger exactly (0 mismatches). The invariants are now a durable offline check: `scripts/python/verify-hp-ledger.py` (read-only vs the SQLite store, exit 0 on both fresh sessions, exit 1 on the pre-fix ledger) plus the synthetic form in the decoder regression test |
...
`cam001-v7-evidence-template.md`. **SUPERSEDED by CAM-013 (2026-08-11):**
the 101–177° gaps were measured against the hull center; with the
turret-level aim point the yaw is tank-aligned (091b: 1.1°) and the
stable-phase captures verify as chase — the non-chase STATE finding was a
measurement-target artifact (only the battle-intro cinematic is genuinely
non-chase).
The
...
closed 2026-08-14:** medvedkovo + savanna each returned 6/6 resolved,
...
the third-person look-at property, with a pitch-convention
...
✅ **CAM-013 (2026-08-11): the live session VERIFIES the W2S seam** —
two launches / three captures (`cam001-v7-aggregate-091/091b/092`):
every round shows the chase signature (identity gates 3/3, basis
coherent, eye yz-swap 2–3 m from the tank, yaw tank-following, memory
pitch level). The earlier non-chase verdicts were a hull-center
measurement artifact — the WoTB chase camera aims at the TURRET-LEVEL
AIM POINT (~1.9 m above the hull center), so pitch-to-AIM ≈ 0° ≈ the
memory pitch. Re-verdict with the aim-point target: **091/091b/092/v7c
verified (all chase), v7b battle-intro cinematic correctly non-chase**;
`w2sProjectionVerified: true`. Validator-only change (no read surface /
offsets). The exact convention is settled: aim point = hull + ~1.9 m
vertical, tank below-center framing.
...
the exact convention — CAM-013 (2026-08-11): turret-level aim point ~1.9 m
above the hull center; the 40/47/64/90° band is kept for the center check.
...
| Step | WS | Deliverable |
|---|---|---|
| P1 | H | Velocity `+0x28` promotion (live-verified semantics) — NOT promoted (G0 record; no package) |
| P2 | H | HP publication — **APPLIED 2026-08-12 (OD-RECOVERY-092)**: `Verified` via the entity-base chain (final hop `recordOffset 184`, signed int16 at `+0xB8`); 2-replay agreement (savanna 087 Strict 8/8 + medvedkovo 091 Phase-4 Strict 4/4, `twoReplayRepeatability = true`) |
| P3 | H | Facing/yaw publication — **APPLIED 2026-08-12 (OD-RECOVERY-092)**: `Verified` via the ring-record chain (`recordOffset 48` = `+0x30`), replacing the quarantined candidate; 2-replay agreement (savanna 088 48/48 + medvedkovo 089 Phase-4 56/56, `twoReplayRepeatability = true`) |
| P4 | H | Pitch/roll publication — **APPLIED 2026-08-12 (OD-RECOVERY-098)**: `Verified` via the ring-record chain (`recordOffset 44` = `+0x2C` pitch, `40` = `+0x28` roll); Phase-4 repeat closed via the rotation-triple reconciliation (`yaw-diff --field pitch\|roll` re-verdicts the SAME OD-088/089 dumps — savanna 48/48 + medvedkovo 56/56 each, `twoReplayRepeatability = true`); the rotation triple is fully published |
| P5 | H | Damage-dealt publication — **APPLIED 2026-08-12 (OD-RECOVERY-097)**: `Verified` via the NEW `vftableScan` chain hop (avatar-stats quad `[avatar+0x118]` dword0); live HITs OD-095/096 (two-replay, `twoReplayRepeatability = true`) + OD-099 (in-session, default lag); consumption committed (live frame's own-row `DamageDealt`) |

### Phase 4 — Overlay 2.0 (data-rich replay overlay)

| Step | WS | Deliverable |
|---|---|---|
| V1 | F | ✅ Facing arrows on nameplates (2026-08-10, replay mode: packet yaw ground truth — `OverlayTankState.YawRadians` was already in the frame; now threaded through projection → API → HUD as `ScreenHeadingDegrees`, perspective-correct two-point probe in `WorldToScreen`, arrow drawn above each nameplate; live mode reuses the same seam — yaw DISCOVERED + live-verified 2026-08-11 (ring-record `+0x30`, OD-RECOVERY-088/089) and the live frame already carries `ScreenHeadingDegrees` (X4-E2E capture)) |
| V2 | F | Objective markers (capture zones) ride the O3 beacon layer; event-feed tie-ins ✅ (2026-08-10): transient damage pips + death markers from the decoded event stream, windowed 2 s, projected over the affected tank's nameplate (`OverlayEventPip` → `Pips` in frame/API/HUD). ✅ **Kill-feed ticker** (2026-08-10): persistent kill list from the Destroyed events with killer attribution (attacker of the victim's last damage within the 3 s posthumous window), bottom-left HUD panel (`OverlayKill` → `Kills` in frame/API/HUD) — verified on savanna: 8 kills with real player names. ✅ **Playback progress bar** (2026-08-10): a thin bottom-centre overlay bar fills with `CurrentTimeSeconds / Duration` and shows a `m:ss / m:ss` time label (`BuildPlaybackBar`, pure `PlaybackFillWidth`/`FormatPlaybackLabel` helpers) — the replay position is now visible on the game overlay itself, complementing the session panel's existing scrubber. ✅ **Scoreboard panel** (2026-08-10): every roster tank's cumulative damage dealt + kills at the frame time, top-right overlay panel sorted by damage (`OverlayTankState.DamageDealt/Kills` — one damage-dealt pass in `BuildFrame`, kills counted from the same attribution as the kill feed — → `ProjectedTank` → API → `ScoreboardItems` → `BuildScoreboard`). ✅ **Damage taken + nameplate totals** (2026-08-10): `DamageTaken` (already computed for `hpFraction`, now reused) threads to a scoreboard column, and each nameplate shows a compact "1,200 dmg · 2 kills" totals line under the HP bar. Verified on savanna t=250: 14 rows, kills sum (8) == kill feed, dead tanks keep final totals, and **total dealt (22094) == total taken (22094)** — every damage event balanced |
...
| V4 | F | ✅ **God-view replay minimap** (2026-08-10): world X/Z now ride through the frame API (`ProjectedTank.WorldX/WorldZ` → `OverlayTankResponse` → CLI), the view model normalizes every roster tank against the session's map boundary (`MinimapMath.Normalize` → `MinimapItems`), and the HUD draws a bottom-right panel — team-colored dots, grey wrecks, white camera ring (`W2sHudView.BuildMinimap`). Pure god-view: tanks behind the camera still appear. ✅ **Beacons on the minimap** (2026-08-10): `ProjectedBeacon.WorldX/WorldZ` → `OverlayBeaconResponse` → `MinimapBeacons` normalized against the same boundary and drawn as colored diamonds under the tank dots. ✅ **Map texture under the dots** (2026-08-10): the minimap image loaded via `LoadMinimapAsync` now renders as the panel background (`MinimapImageSource` → `Render` → `BuildMinimap`), stretched to the panel square so normalized dots align with terrain; dot alignment is the invariant (a non-square boundary distorts texture, not dots). Fails closed when no texture is installed. ✅ **Texture resolution gap CLOSED (2026-08-14):** `MapMetadata.SceneResourcePath` now preserves the exact-build `maps.yaml.dvpl` `localName`, and `MinimapTextureService` resolves numeric replay arena IDs through that metadata before selecting a name-based folder. Numeric variants such as `desert_train_02` remain intact; unknown numeric IDs and invalid path components fail closed. Synthetic service coverage pins arena-id → scene-path → variant-folder → WebP → PNG, and a read-only installed-game check proved arena `1` resolves to the Karelia/Rockfield texture and returns a valid PNG. This exact install still ships no savanna (`11`) or medvedkovo (`7`) texture, so those two replays honestly remain dots-only; dot-versus-terrain HUD alignment awaits a replay for a texture-bearing map. ✅ **Camera facing tick** (2026-08-10): `CameraYawRadians` flows to the view model (`MinimapCameraYawRadians`) and the HUD draws a small white triangle from the camera ring toward the viewpoint's facing (packet yaw convention 0=+Z, +π/2=+X mapped to panel pixels; `CameraTickApex` pure helper). Verified on savanna: 14 tanks normalized against map-11 boundary; a center POI beacon normalizes to (0.56, 0.57); camera yaw 0.564 rad at t=100 renders a right-down tick |
...
| X2b | C | **Live roster enumeration — team-based split evidenced 2026-08-11 (OD-RECOVERY-086):** `/discover/entity-roster` full-tree walk + movement-filter vtable gate returns the player's OWN team's avatar family only (7/14, precision 1.000, recall 0.500, 0 extra — all found = team 1, all missing = team 2/enemies). The X4 loop must re-enumerate per tick or add a second discriminator for enemy avatars — **SUPERSEDED for enemy ids by X4-E2E (2026-08-11): the per-tick re-enumeration DID catch the full 7v7 roster at battle start and every enemy id joins exactly** (mid-battle coverage remains enumeration-dependent); design `docs/operations/live-roster-read-design.md` | X1 |
| X3 | C/D | Camera offset track (the LIVE game's camera — a new discovery target) | X1 |
| X4 | F | ✅ **LiveFrameSource — IMPLEMENTED 2026-08-11** (design `docs/operations/live-frame-loop-design.md`): coordinator-composed `POST /discover/live-frame` (roster → batch regions → camera pose, ONE guarded reader lease + ONE G2 clock label, `hp: null` until L1) → pure `LiveFrameProjector` → `GET /api/v1/live/frame` serving the SAME `OverlayFrameResponse` the replay path uses (shared `ToOverlayFrameResponse`, no overlay rewrite) → overlay `IsLiveMode` toggle → `LiveFrameReadMeasurement` read-pass window (item-7 budget). The 086 rehearsal is DONE (X2 batch PASS live + X3 team-split evidenced — the loop must re-enumerate per tick or add an enemy discriminator). L2 facing is DONE (088 savanna HIT at `+0x30`, 089 medvedkovo Phase-4 HIT — `twoReplayRepeatability = true`), L1 HP is DONE (087 HIT at `+0xB8`; the entity-base HP read is WIRED into the frame, additive). CAM-001 v7 camera evidence DONE (CAM-013 verified the W2S seam live — `w2sProjectionVerified: true`, 091/091b/092 chase; only the battle-intro cinematic is non-chase). OD-RECOVERY-090 (L3 damage-dealt) DONE — honest-negative in the entity records; the lane then CLOSED in the avatar family: quad `+0x0` live-HIT on both replays (OD-RECOVERY-095/096, `twoReplayRepeatability = true`), PUBLISHED as the G2 `vftableScan` chain (OD-097), re-proven live in-session at default lag (OD-099, session `019ff74f`), and the CONSUMPTION is committed — the live frame's own-row `DamageDealt` reads the published chain via the coordinator's avatar-stats anchor (fail-closed null; projector own-row only). Plan record: `docs/operations/l3-damage-dealt-avatar-family-plan.md`. The id→name join is IMPLEMENTED (2026-08-11, per-id best-effort within the design's gate: `LiveFrameProjector` optional participant map + `GET /live/frame?sessionId=` with the same first-match convention as the replay path + the overlay forwards its selected session in live mode; 6 new tests; design `docs/operations/live-roster-name-join-design.md`). **Enemy ids are JOINABLE — proven live (X4-E2E, 2026-08-11):** the movement-filter family is time-varying; the loop's per-tick re-enumeration caught the FULL 7v7 roster at battle start and the join resolved every enemy id exactly (14/14, 0 mismatches vs the independent decoded roster; mid-battle frames enumerate own-team 7 — coverage is enumeration-dependent, not a join limitation). **End-to-end overlay LIVE-mode validation DONE (X4-E2E, 2026-08-11):** a mid-battle live frame was verified on the CAM-013 seam — 15 tanks, 14/14 exact joins, 13/15 real HP, chase camera (eye 1.9 m, pitch level = aim point, tank below-center); CAM-003 controller flip re-demonstrated fail-closed (`409 playback-controller-vtable`); handoff `2026-08-11-live-frame-end-to-end.md`. **Own-nameplate suppression DONE (2026-08-11, name-join step 4 second half):** `OverlayFrameResponse.OwnEntityId` (additive nullable) + the live endpoint resolves `Session.ViewpointParticipantId` → participant `EntityId` (fail-closed null) + the overlay skips exactly that nameplate (own tank still reaches the minimap); the `< 1.0 m` distance heuristic stays as the replay/unknown fallback. **Own-tank edge marker DONE (2026-08-12):** when the own tank projects off-viewport the HUD draws a clamped edge chevron back at the hull (`OwnMarkerItem`/`OwnMarkerMath`, 8 tests; fail-closed: unknown id / on-screen / missing projection → no marker). **Live frame replay clock WIRED + LIVE-VERIFIED (2026-08-12):** the endpoint forwards the session id into the discover request so the batch core's ONE G2 replay-clock snapshot runs (the X4-E2E capture's `replayTimeSeconds: 0` was the un-forwarded session id). Live verification (sessions `019ff36c`/`019ff370`) read **115.5 s** mid-battle and exposed a pre-existing launcher anchor-date bug — the anchor DATE came from the blitz-log FILENAME (local date) while the marker's leading time is UTC, so ANY local-evening launch (e.g. 19:43 local = 00:43 UTC next day at UTC-5) writes its marker to the previous day's file and parses the anchor 24 h stale (frame read 86517 s); fixed in `launch-offline-replay-for-od.ps1` with a bounded date roll to within 10 min of UTC now (2nd launch anchored 08-12T00:48:20Z, frame 115.5 s). Unknown/stale session still fails closed to 0.0 | X2 + X2b + X3 |
...
| Step | WS | Deliverable |
|---|---|---|
| PN-1 | G | **Static-data extraction** — **PROBED 2026-08-13: present + readable.** Vehicle XML (`Data/XML/item_defs/vehicles/{nation}/{tank}.xml.dvpl`) carries per-group hull + turret armor; `components/shells.xml.dvpl` carries caliber/kind/normalizationAngle/ricochetAngle; `components/guns.xml.dvpl` carries `piercingPower` (2-point range) + speed + maxDistance — all DVPL/LZ4 → XML via the existing `DvplReader`. Collision geometry **PARSED 2026-08-13** (`CollisionMeshParser` + `CollisionRaycast`, verified on the real Churchill mesh) — the per-plate NORMAL is available. **Per-plate THICKNESS mapping RESOLVED 2026-08-13: NOT FEASIBLE** (armor XML has no face geometry; collision `.scg` = 2–11 physics parts, not armor plates; the per-plate `Hull.model`/`Turret.model` hit-test models are not shipped loose). Badge stays honest-nominal: front via `primaryArmor`, side/rear fail-closed Unknown |
| PN-2 | F | **Pen math module** — **DONE 2026-08-13** (`Core/Overlay/ArmorPenetration.cs`, 18 tests): ray-plane hit, incidence, effective armor = thickness/cos(angle), ≥70° ricochet on the RAW angle (+3× overmatch), 5°/2°/0° normalization with the two-caliber amplification, pen-at-range, banded green/yellow/red verdict, and the `piercingPower` 2-point range mapping (`ShellSpec.FromPiercingPower`). Mechanics corrected against the official support article (ricochet ordering + ±5% penetration RNG, not ±25%) |
| PN-3 | F | **Replay-mode HUD** — aim = the CAM-013-verified camera pose (no new discovery; the replay camera drives the turret); pen badge (colored + numeric) on the aimed enemy's nameplate. **DONE 2026-08-13** end-to-end (frame → response → WPF badge); the badge uses the collision mesh's true surface normal when one is parsed. Honest limits: front-only armor, stock AP shell, thickness still nominal |
| PN-4 | F | **DONE 2026-08-14** — offline scorer plus repeat live CAM-013 aim proof across two content-distinct replays; medvedkovo improved 69.565% -> 72.727% and savanna/Churchill improved 38.889% -> 46.667% band accuracy; 150 + 161 G2-proven samples, per-shot deltas in the PN-4 handoffs |
| PN-5 | F | **DONE for the `v0.2.0-alpha` ship-ready prototype 2026-08-14** — live badge and scorer consume the CAM-013 chase-camera aim; T1 turret/gun traversal remains optional exact lock-on research |
