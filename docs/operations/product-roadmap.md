...
| T1 | D | **Turret-facing + lock-on discovery (survey DONE 2026-08-11)** — the entity base and ring record carry hull rotation ONLY; turret/lock-on have NO replay ground truth, so discovery is live-behavioral: capture the entity-base region while the player traverses the turret without moving the hull (a candidate field responds; hull yaw/pitch/roll stay put). The survey also MEASURED the entity-base copy layout (id `+0x1C`, position `+0x3C/40/44`, rotation triple `+0x48/4C/50`, HP `+0xB8/BA/11C`) — a single-region per-tank read candidate (unproven as canonical). See `record-diffing-groundwork.md` §Turret-facing | 1 |
