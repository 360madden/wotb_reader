...
| L1 | D | HP live session (Oasis Palms victim 3760578; anchor **entity-base**, region ≥ 0x120, correlate **int16** — `VerifyPlayerHpChain` 2026-08-11 (**26/26**) pins the entity-base map: strafing `+0x7C`, gun angles `+0x7E`, current int16 `+0xB8`, alive `+0xBA`, engine-mode ptr `+0xBC`, hit marks `+0xC8`, mask pair `+0xD4`/`+0xD8`, critical devices `+0xE0`, destroyed devices `+0xEC`, active equipments `+0xF8`, debug strings `+0x110`, **max int16 `+0x11C`**, healing int16 `+0x11E`; the `+0x48` from rehearsals is a SYNTHETIC FIXTURE, refuted; `hp-diff --int16 true` is the HP default) | 1 |
