...
|----------|------------|---------------|
| `decoder-auditor` | `claude-opus-5-thinking-max` | temporary tracked-source export; no replay files |
| `security-auditor` | `claude-fable-5-thinking-xhigh` | temporary tracked-source export; `NO ZDR`, so no private data |

Cursor Agent CLI sandboxing is unavailable on Windows. The adapter compensates
with Ask mode, a standalone export of committed `HEAD`, project-level denial of
all shell/write tools, and no force/MCP/cloud-handoff switches. Do not invoke
the CLI directly for repository work.
