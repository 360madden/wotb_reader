...
- Full gate, required before milestone commits: `scripts/validate.ps1` — locked restore → `dotnet format --verify-no-changes` → Release build → all tests → `scripts/scan-repository.ps1` (secret + ignore-policy scan) → PSScriptAnalyzer hygiene gate (`install-psscriptanalyzer.ps1` + `invoke-scriptanalyzer.ps1`). Add `-AuditPackages` for the transitive vulnerability audit.
...
- **PowerShell 5.1 reads BOM-less UTF-8 as ANSI.** Any non-ASCII byte in a `.ps1` silently corrupts at runtime (an em-dash's trailing byte `0x94` maps to `"` in ANSI and terminates a string literal early). Keep every `.ps1` ASCII-only. The PSScriptAnalyzer gate (`scripts/invoke-scriptanalyzer.ps1`, wired into `validate.ps1` + CI) enforces this and the repo custom rules (`tools/psscriptanalyzer-custom-rules.psm1`) ban the `[double]::IsFinite` and `??`/`&&`/`||` classes. If you add a custom rule, type its parameter with a **concrete AST node** (`[ScriptBlockAst]`), never `[Ast]` — analyzer matches by type-name substring and silently never invokes `[Ast]`-typed rules — and remember `-CustomRulePath` replaces defaults unless `-IncludeDefaultRules` is passed. Run `powershell -File scripts/invoke-scriptanalyzer.ps1 -SelfTest` after touching the rules module.
