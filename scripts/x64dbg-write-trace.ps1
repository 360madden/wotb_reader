...
# FRESH29b: parse ODWT_HIT lines from a file (the reliable capture channel)
# and merge unique 'addr rip' keys into the target list. Returns the count
# of NEW keys added. Shared by the poll loop and the post-window harvest so
# the three copies cannot drift.
function Add-OdwtHitLines {
    param([string]$Path, [System.Collections.Generic.List[string]]$ToList)
    ...
    foreach ($ln in @(Get-Content -LiteralPath $Path -ErrorAction SilentlyContinue)) {
        if ($ln -match 'ODWT_HIT addr=(0x[0-9a-fA-F]+) rip=(0x[0-9a-fA-F]+)') {
            $hitKey = ($Matches[1] + ' ' + $Matches[2])
            if ($ToList -notcontains $hitKey) {
                $ToList.Add($hitKey)
                $added++
            }
        }
...
try {
    ...
    # FRESH29b: the HitsDir is a FIXED shared path across runs - delete any
    # stale capture files from a previous session BEFORE this run, or a
    # previous run's ODWT_HIT lines (if the debugger appends rather than
    # truncates) would be harvested as false hits.
    Get-ChildItem -LiteralPath $HitsDir -File -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -match '^odwt-.*\.bin$|^od-wt-(engine|bp)\.log$' } |
    ...
    # FRESH29b: capture the x64dbg ENGINE LOG to a real FILE. The FRESH9
    # probe campaign proved file capture (setlogfile + SetBreakpointLogFile)
    # produces ODWT_HIT evidence, while the UIA Log-tab read is LOSSY (its
    # own comment: "returned 0 lines in clean-rig runs") and the savedata
    # command is flaky across builds. FRESH29 live: the armed x-field moved
    # 20.94 units during the window (values_changed=true, world advancing)
    # yet hits=0 - the memory BP fired (game paused: window_cpu_delta_ms=
    # 4375 in a 25s window vs ~25s when running) but the evidence was never
    # written to disk. The engine-log file makes the hit capture reliable.
    $engineLog = Join-Path $HitsDir 'od-wt-engine.log'
    $bpLogFile = Join-Path $HitsDir 'od-wt-bp.log'
    [void]$scriptLines.Add(('setlogfile "{0}"' -f $engineLog))
    foreach ($a in $armed) {
    ...
    Write-Wt ("engine_log_file=" + $engineLog)
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        # FRESH29b: file-based BP log harvest (the reliable channel - the
        # FRESH9 probe proved SetBreakpointLogFile produces evidence; the
        # UIA Log-tab read is lossy). The engine log file is checked too,
        # so a write that paused the debuggee is counted even if the per-BP
    ...
    # Harvest the ODWT_HIT evidence. FRESH29b: the PRIMARY channel is now the
    # FILE-based BP log (SetBreakpointLogFile + setlogfile in the script) -
    # the FRESH9 probe proved it writes {rip}-substituted ODWT_HIT lines to
    # disk reliably, while the UIA Log-tab read is lossy and savedata is
    # flaky across builds. FRESH29 live proved the memory BP fired (game
    # paused at window_cpu_delta_ms=4375 in 25s while the armed field moved
    # 20.94 units) but the old capture channels lost every hit.
    $harvested = New-Object System.Collections.Generic.List[string]
    $harvestSource = 'uia-tab'
    # FRESH29b: file-based channels first (reliable); the lossy UIA Log-tab
    # read is the last resort. Track the ACTUAL source that produced hits,
    # not which file merely exists.
    $nBp = Add-OdwtHitLines -Path $bpLogFile -ToList $harvested
    if ($nBp -gt 0) { $harvestSource = 'bp-log-file' }
    else {
        $nEng = Add-OdwtHitLines -Path $engineLog -ToList $harvested
        if ($nEng -gt 0) { $harvestSource = 'engine-log-file' }
        else {
            # Last resort: the lossy UIA Log-tab read.
            try {
                foreach ($ln in @(Read-X64DbgLog -Handle $win.Handle)) {
                    if ($ln -match 'ODWT_HIT addr=(0x[0-9a-fA-F]+) rip=(0x[0-9a-fA-F]+)') {
                        $hitKey = ($Matches[1] + ' ' + $Matches[2])
                        if ($harvested -notcontains $hitKey) { $harvested.Add($hitKey) }
                    }
                }
            }
    ...
    Write-Wt ('log_harvest_hits=' + $harvested.Count + ' source=' + $harvestSource)
