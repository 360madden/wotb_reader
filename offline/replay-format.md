...
| 32 | 258 | entity-id + event flag + payload, 3 layouts (11 B, 25–27 B, 14 B) | **damage/impact event mirror (2026-08-10)**: fires at the same instants as the type-8 direct-damage events for the same victim (81/85 alignment on Oasis, 107/120 on Dead Rail — every miss is an amt=0/no-damage event) and embeds the SAME 6-byte shell signature as its matching type-8 packet (e.g. `a6 a5 e0 a2 a8 b1` at t=69.13, `ff e0 b9 d7 d8 98` at t=69.62). Flag prefixes distinguish the event (`01 11`/`01 12` = damage-with-payload, `01 02`/`01 03` = short companion, `00 10` = state snapshot at spawn and at the end of the victim's event chain); shell/effect entities (0x30xxxx range) carry `01 05`/`01 06`. NOT spotting: no reveal/visibility data in any payload. **Layout pinning (2026-08-13):** the mirror is persisted in the store's raw records (66 386 packets) and extractable offline. Payload = entity-id u32 + flag u16 + a `00000098` (for `01 11`) / `00000099` (for `01 12`) marker word + a 1–3 byte flags region + the 6-byte shell signature + two 4-byte trailing fields. **Honest negative:** the flag prefix does NOT separate pen from bounce — the same `01 12` fires for both (victim 3760569: damage at t=69.62/86.12/91.00 and no-damage at t=79.93/86.63), and a candidate bit-7 flags byte at payload +0x0A is not a clean discriminator (bit7 set: 24 dmg/3 nodmg; clear: 16 dmg/13 nodmg). The damage-vs-bounce field is in the payload but not yet pinned. |
