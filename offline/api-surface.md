...
| `/discover/entity-regions` | POST | **Batch** region dumps (≤ 16 entities, ≤ 16 KB total) in one round trip with ONE replay-clock attestation — the per-frame live read surface (design: `docs/operations/batch-entity-read-design.md`); response carries the read-pass measurement window; each item may also request an `entityBaseRegionLength` (the L1 HP read at the entity base, same resolve + ONE attestation) with its own bytes/failure in the response |
...
| `/discover/live-frame` | POST | **One composed per-frame read** — roster enumeration → ring-record + entity-base batch (position `+0x10`, hull yaw `+0x30`, HP current `+0xB8` / max `+0x11C` — all live-verified 2026-08-11) → own-row `DamageDealt` via the avatar-stats anchor (G2 chain, 2026-08-12) → CAM-001 camera pose, all under ONE guarded reader lease with ONE G2 clock attestation; ids-only; per-tank HP stays honest-null when its entity-base read failed (design: `docs/operations/live-frame-loop-design.md`) |
| `/live/frame` | GET | **LiveFrameSource seam** — the composed live frame projected to viewport pixels in the SAME `OverlayFrameResponse` shape the replay frame uses (shared `ToOverlayFrameResponse` mapping), so the HUD renders live nameplates without touching its render path; HP bar real when the L1 entity-base read resolved, empty/unknown only when it failed; own-row `DamageDealt` real (fail-closed null on any read failure); replay clock real when the endpoint forwards the session id (launcher G2 anchor); non-resolved → 409, read failure → 503 |
