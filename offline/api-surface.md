...
| `/discover/live-frame` | POST | **One composed per-frame read** — roster enumeration → ring-record batch (position `+0x10`, hull yaw `+0x30` — live-verified 2026-08-11) → CAM-001 camera pose, all under ONE guarded reader lease with ONE G2 clock attestation; ids-only, `hp` honestly null until L1 (design: `docs/operations/live-frame-loop-design.md`) |
| `/live/frame` | GET | **LiveFrameSource seam** — the composed live frame projected to viewport pixels in the SAME `OverlayFrameResponse` shape the replay frame uses (shared `ToOverlayFrameResponse` mapping), so the HUD renders live nameplates without touching its render path; hp unknown (empty bar) until L1; non-resolved → 409, read failure → 503 |
