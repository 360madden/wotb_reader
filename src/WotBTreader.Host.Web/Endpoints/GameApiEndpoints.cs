...
internal static class GameApiEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var snapReq = new MemorySnapshotRequest(
            Math.Clamp(request.ValueSize, 2, 8),
            request.FloatMin, request.FloatMax,
            request.IntMin, request.IntMax,
            request.MinAddress, request.MaxAddress);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!result.IsSuccess)
            return Results.BadRequest(new { error = result.Error?.Code, detail = result.Error?.Message });
        var r = result.Value!;
