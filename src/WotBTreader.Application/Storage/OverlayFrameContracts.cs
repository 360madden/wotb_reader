...
public interface IOverlayFrameSource
{
    ...
    /// <paramref name="replayTime"/>: the viewpoint camera plus every tank
    /// with a nearest position sample at or before that time. Tanks without
    /// position evidence at the frame time are omitted; the frame is never
    /// fabricated from endpoints outside the sample span.
    /// </summary>
