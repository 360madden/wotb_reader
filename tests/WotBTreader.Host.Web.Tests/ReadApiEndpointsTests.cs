...
public sealed class ReadApiEndpointsTests
{
    ...
    [TestMethod]
    public async Task LiveFrame_ExcludesAlliedTankFromPenBadge()
    {
        // The live badge scores ENEMY tanks only: the own team comes from the
        // decoded viewpoint participant, and a same-team tank directly under
        // the aim produces no badge (allies are never penetration targets).
        CameraScannerStub scanner = new(
            OperationResult.Failure<CameraPoseReadResult>(
                new ApplicationError("unused", "Unused.")),
            OperationResult.Success(new LiveFrameReadResult(
